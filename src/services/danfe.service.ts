import { parseXmlToJson } from "../utils/xml-parser";
import { NfeResponse } from "../types/nfe-types";
import { DocumentType, NfeModel, NfeType } from "../enums/danfe-enums";

export const DanfeService = {
  xmlToJson: async (xmlContent: string) => {
      ...
      if (!Array.isArray(nfe.det)) {
        formattedJson.products.push({
          ...
          taxes: {
            ...
            cofins: {
              value:
                nfe.det.imposto?.COFINS?.COFINSAliq?.vCOFINS?._text || "0.00",
              rate:
                nfe.det.imposto?.COFINS?.COFINSAliq?.pCOFINS?._text || "0.00",
            },
          },
        });
      } else {
        nfe.det.forEach((item: any) => {
          formattedJson.products.push({
            ...
            taxes: {
              ...
              cofins: {
                value:
                  item.imposto?.COFINS?.COFINSAliq?.vCOFINS?._text || "0.00",
                rate:
                  item.imposto?.COFINS?.COFINSAliq?.pCOFINS?._text || "0.00",
              },
            },
          });
        });
